...
# -*- coding: utf-8 -*-
"""
ROS1 Python 示例：左转一小段 → 自动停止
- 体现内容：向 /ackermann_cmd 发布低速左转命令（转向角为正），持续 T 秒后自动停止
- 安全提示：保持低速与短时长；如转向过急或停不稳，立即调小参数

默认参数（可通过 rosparam 修改）：
- ~speed_mps = 0.8
- ~steering_angle_deg = 40.0  # 左转为正角度（度），内部转为弧度
- ~duration_sec = 1.0
"""
...
def main():
    rospy.init_node('l2_2_left_stop')
# 获取参数
...
#发布转向命令
    ...
    rospy.loginfo('Left-turn start: speed=%.3f m/s, steering=%.3f deg (%.3f rad), duration=%.2f s',
