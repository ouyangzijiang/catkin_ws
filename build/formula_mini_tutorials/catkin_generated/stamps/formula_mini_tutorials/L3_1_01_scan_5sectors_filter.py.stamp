...
def sector_min(scan, center_deg, half_deg, min_valid, max_valid):
    '''在指定扇区内计算最小有效距离。scan: LaserScan消息对象，包含激光雷达的完整数据
    center_deg: 扇区中心角度（度）
    half_deg: 扇区半宽度（度）
    min_valid/max_valid: 有效距离的范围阈值'''
    #   角度转换为弧度
...
class FiveSectorsNode(object):
    ...
    def __init__(self):
        ...
        self.half_deg = float(rospy.get_param('~half_width_deg', 15.0))
        self.min_valid = float(rospy.get_param('~min_valid_m', 0.05))
        self.max_valid = float(rospy.get_param('~max_valid_m', 20.0))
        # 订阅激光扫描数据---接收到消息后触发回调
