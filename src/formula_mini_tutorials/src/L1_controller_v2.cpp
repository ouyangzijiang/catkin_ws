...
L1Controller::L1Controller()
{
    ...
    pn.param("lrv", lrv, 10.0);
    pn.param("goal_radius", goal_radius, 0.05);  
    ...
    pn.param("base_angle", base_angle, 0.8);
    ...
    if(!goal_reached){
        for(int i =0; i< map_path.poses.size(); i++)
        {
            ...
            try
            {
                ...
                // Fix: Log position to avoid nesting errors
                ROS_INFO("WP %d: Fwd? %d | Dist>=Lfw(%.2fm)? %d", 
                         i, _isForwardWayPt, Lfw, _isWayPtAwayFromLfwDist);

                if(_isForwardWayPt && _isWayPtAwayFromLfwDist)
                {
                    forwardPt = odom_path_wayPt;
                    foundForwardPt = true;
                    ROS_INFO("Found forward WP! x=%.2f, y=%.2f", forwardPt.x, forwardPt.y);
                    break;
                }
            }
            catch(tf::TransformException &ex)
        ...
        // Fix: Fallback logic - use goal if no forward point found
        if(!foundForwardPt)
        {
            ROS_WARN("No valid forward WP found, using goal as fallback!");
            double dist2goal = getCar2GoalDist();
            if(dist2goal > 0.05) // Use fallback only if distance > 0.05m
            {
